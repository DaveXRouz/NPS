...
When a new Claude session opens this project and wants to start fixing issues:

1. Read `CLAUDE.md` (boot sequence)
2. Read this file (`WISH_LIST.md`) — understand what's broken and in what order to fix it
3. Read `ISSUE_LIST.md` for the full detail on any specific issue
4. Pick a priority tier and start fixing

---

## Project State Summary (as of 2026-02-19)

### What Works

- PostgreSQL database schema — fully implemented, migrations 1–14 applied
- FastAPI authentication — JWT + API key, refresh tokens, brute-force protection
- Frontend scaffolding — 20+ React components exist and render (mostly)
...
- `Vault.tsx` — page exists, no route in App.tsx (Issue #124)
- Scanner API — 7 endpoints, all TODO stubs (Issue #128)
- Vault API — 4 endpoints, all TODO stubs (Issue #127)
- Legacy migration scripts — 5 scripts, all TODO stubs (Issue #129)
...
# Check for text-nps-bg-danger (should be zero after Issue #8 fix)
grep -rn "text-nps-bg-danger" frontend/src/

# Check for undefined Tailwind classes
grep -rn "bg-nps-bg-button\|bg-nps-bg-elevated" frontend/src/

# Check for bare except (should be zero after Issue #132 fix)
grep -n "except Exception: pass\|except:$" api/app/routers/oracle.py
```

---

## Session Bootstrap — Starting a Fix Session

When you open a new session to fix issues from this list:

```
1. Read CLAUDE.md (boot sequence — mandatory)
2. Read this WISH_LIST.md (you're reading it now)
3. Choose a priority tier:
   - P0 if any security keys not yet rotated
   - P1 if Blocker 1 (Oracle backend) or Blocker 3 (AdminGuard) not fixed
   - P2 for polish work
4. Pick specific issues from the tier
5. Show plan to Dave → wait for approval
6. Execute per CLAUDE.md rules:
   - Format code after every file change
   - Run tests before declaring done
   - Git commit with session tag [layer] description (#issues-NNN)
   - Update SESSION_LOG.md at end
```

**Recommended first session**: Fix P0 security issues (#121, #122, #123) — rotate keys, remove hardcoded defaults, add to gitignore. Then fix Blocker 3 (#11 AdminGuard wiring — 2 lines). Then fix the Quick Win list (20+ trivial issues resolved in one session).
...
When all 9 phases are complete alongside ISSUE_LIST.md fixes:
