...
describe("NameReadingForm", () => {
  ...
  it("clicking Use Profile Name pre-fills input", () => {
    render(<NameReadingForm userName="Alice Johnson" onResult={onResult} />);
  ...
  it("submitting calls oracle.name with correct params", async () => {
    render(<NameReadingForm userId={1} onResult={onResult} />);
    ...
    await waitFor(() => {
      expect(mockNameApi).toHaveBeenCalledWith("Alice", 1, "pythagorean");
    });
  });
